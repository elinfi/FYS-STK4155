...
def cross_validation(n, maxdegree, noise, n_folds, method, lmbda, seed):
    ...
    z_test = np.ravel(FrankeFunction(x_test, y_test) + noise*np.random.randn(np.shape(x_test)[0], x_test.shape[0]))
    ...
    for degree in range(0, maxdegree):
        ...
        for k in range(n_folds):
            ...
            if method == OLS:
                ...
                clf_lasso = skl.Lasso(alpha = lmbda).fit(X_train_fold_scaled, z_train_fold)
