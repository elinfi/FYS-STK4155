...
for i, n_batches in enumerate(array_batches):
    X_minibatches = np.split(X_train, n_batches)
    print(i)
...
for i, n_batches in enumerate(array_batches):
    ...
    for j, epoch in enumerate(array_epochs):
        ...
        mse_heatmap[i, j] = f.MSE(z_test, network.layers[-1].a)
...
heatmap.set_title('Leaky RELU + Identity', size=16)
plt.show()
