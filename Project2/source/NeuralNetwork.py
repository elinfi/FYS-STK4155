...
class NeuralNetwork:
    ...
    def create_layers(self, activation, output_activation, seed=7053, no_hidden=False):
        ...
        if no_hidden:
            ...
            self.layers.append(DenseLayer(self.n_inputs, int(self.neurons[0]),
                                          activation, seed))
            # hidden layers
            for i in range(len(self.neurons) - 1):
                self.layers.append(DenseLayer(int(self.neurons[i]),
                                              int(self.neurons[i + 1]),
                                              activation, seed))
            # last hidden layer to output layer
            self.layers.append(DenseLayer(int(self.neurons[-1]), self.n_outputs,
                                          output_activation, seed))
